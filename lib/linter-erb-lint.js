...
export default {
  activate() {
    ...
    this.subscriptions.add(
      atom.config.observe('linter-erb-lint.executablePath', (executablePath) => {
        this.executablePath = executablePath;
      }),
      atom.config.observe('linter-erb-lint.disableWhenNoConfigFile', (disableWhenNoConfigFile) => {
    ...
    if (atom.inDevMode()) {
      ...
      console.log(`linter-erb-lint: Command path: ${this.executablePath}`);
    ...
    return {
      ...
      lint: async (editor) => {
        ...
        try {
          ...
          atom.notifications.addInfo('Linter-ERB-Lint: Linter timed out', {
        ...
        if (output.exitCode === 1) {
          parseFromStd(output.stdout).forEach((result) => {
            messages.push({
              filePath,
              type: 'Warning',
              severity: 'warning',
